...
const ACCESS_KEY = '283a508f555f549676a6c4ec1258ca20';
...
describe('Weatherstack API Cypress Tests', () => {
  ...
  //       // Дозволяємо невелике відхилення, наприклад 0.1 градус
  ...
  //   cities.forEach((city) => {
  //     it(`GET current weather for ${city}`, () => {
  //       cy.request({
  //         method: 'GET',
  //         url: `${API_BASE}/current`,
  ...
  // it('10. Check rate-limit headers presence', () => {
  //   cy.request({
  //     method: 'GET',
  //     url: `${API_BASE}/current`,
  //     qs: { access_key: ACCESS_KEY, query: 'Kyiv' },
  //   }).then((res) => {
  //     expect(res.status).to.eq(200);
  //     // перевіряємо, що присутні заголовки ліміту (мінімум один)
  //     expect(res.headers).to.satisfy(
  //       (h) => h['x-ratelimit-limit-minute'] || h['x-ratelimit-limit']
  //     );
  //   });
  // });
});
